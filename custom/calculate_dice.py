...
def calculate_metrics(gt_mask, pred_mask):
    """Calculate Dice, Precision, and Recall between two binary masks. Masks are expected to be 0 or 255 (or boolean).
...
def evaluate_folder(gt_dir, pred_dir, output_csv):
    """Evaluate Dice, Precision, Recall between GT and Pred folders. Matches files by name.
    """
    ...
    for pred_path in tqdm(pred_files):
        ...
        if not os.path.exists(gt_path):
            ...
            results.append(
                {"case_id": stem, "dice": np.nan, "precision": np.nan, "recall": np.nan, "status": "missing_gt"}
            )
            continue
        ...
        if pred_mask is None or gt_mask is None:
            ...
            results.append(
                {"case_id": stem, "dice": np.nan, "precision": np.nan, "recall": np.nan, "status": "read_error"}
            )
            continue
...
def main():
    ...
    parser.add_argument(
        "--gt_root",
        type=str,
        default="/home/bongmedai/Endo/datasets/medai_endo_data/masks_converted",
        help="Root of GT masks",
    )
    parser.add_argument(
        "--pred_root", type=str, default="/home/bongmedai/Endo/sam3_results", help="Root of Predicted masks"
    )
